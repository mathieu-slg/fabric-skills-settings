...
DISCOVER_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    ("server/content/memory/**/*.md", "memory"),
    ("server/content/**/*.md", "content"),
    ("server/skills/*/SKILL.md", "skill"),
    ("server/skills/*/sections/*.md", "content"),
    # Legacy target-repo layout (kept so the runtime CRUD path still works in
    # target repos that haven't migrated to the server-only model yet).

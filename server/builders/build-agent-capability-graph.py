...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="output path (default: <target>/dist/.graph/agent-capabilities.json)",
    )
    parser.add_argument(
        "--html",
        type=Path,
        default=None,
        help="HTML output path (default: <target>/dist/.graph/agent-capabilities.html)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_path = args.out or (root / "dist" / ".graph" / "agent-capabilities.json")

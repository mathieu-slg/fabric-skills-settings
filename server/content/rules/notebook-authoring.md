...
`SOURCE_DIR` in a downstream notebook must exactly match `OUTPUT_DIR` in the upstream notebook. A path mismatch produces a silent empty read — no error, just missing data. After any staging-path change, call the `pipeline_lineage_check` MCP tool with the notebooks uploaded as `{relative_path: file_content}`:
...
pipeline_lineage_check(
    ...
    topic="<topic>",  # optional; default checks all uploaded topics
)
```

A FAIL must be fixed before building or deploying. The tool returns the full validator output including any Python traceback so the offending notebook can be located precisely.

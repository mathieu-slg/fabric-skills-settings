...
| Workspace registry | `workspaces.json` exists in project root | Run `python tool/workspace/init.py` from the project root — queries the Fabric API with the local SPN and writes the complete workspace and resource registry. |
| Active workspace set | `workspaces.json` has an `"active"` field that is not null | Run `python tool/workspace/switch.py list` to see options, then `python tool/workspace/switch.py <displayName>` to choose. |
...
A workspace confirmation covers the whole session. Re-confirm only after the human explicitly runs `python tool/workspace/switch.py`.
...
Humans must create the Fabric workspace and any **lakehouse** items first. Agents may create or update **notebook** items and workspace folders automatically via `python tool/notebook/deploy.py`.

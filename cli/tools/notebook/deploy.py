...
def _resolve_fab_command() -> list[str]:
    """Locate the `fab` executable. Uses shutil.which so it works on any
    platform — picks up fab.exe on Windows and fab on Unix, wherever
    `uv tool install ms-fabric-cli` placed it."""
    import shutil

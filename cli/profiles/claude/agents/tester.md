...
Validate independently. The **fabric-validate** skill is owned by tester; fetch its workflow with `graph_get_node('skills/fabric-validate')` before writing or running DQ checks. Use `graph_get_node('skills/fabric-ops')` to look up lakehouse-inspection patterns when checking for schema drift or contract alignment, and run `python tool/lakehouse/list-tables.py` from the project root to read current schemas.

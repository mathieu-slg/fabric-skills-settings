...
For workspace inventory, refresh the registry with `python tool/workspace/init.py` from the project root (it queries the Fabric API with the user's SPN) and read `workspaces.json`. The SBOM and platform inventory are stored as graph memory nodes — fetch them via `graph_get_node('memory/sbom')` and `graph_get_node('memory/platform')` (or `graph_search` if the exact id is unknown).
...
- Run `python tool/workspace/init.py` to refresh `workspaces.json`, then read it to enumerate workspace items and confirm access scope.
...
- Refresh `workspaces.json` via `python tool/workspace/init.py` and read it to confirm inventory completeness against the live Fabric tenant
